        ...
        map.on('ping', (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
            ...
            .on('join', (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                joinUserChannel(msg)
                    .then(connection => {
                        ...
                        msg.channel.send(createInfoEmbed(`Dołączono na kanał: ${connection.channel.name}`));
            ...
            .on('leave', (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                ...
                this.client.voice.connections.forEach(conn => {
                    conn.disconnect();
                    msg.channel.send(createInfoEmbed(`Wychodzenie z kanału: ${conn.channel.name}`));
                });
            })
                    ...
                    if(!this.player.connection) {
                        joinUserChannel(msg)
                            .then(conn => {
                                ...
                                msg.channel.send(createInfoEmbed(`Dołączono do kanału: ${conn.channel.name}`));
                                done();
                            });
                    } else
                        done();
            ...
            .on('time', (cmd: SuccessfulParsedMessage<Message>, msg: Message) => {
                ...
                if(this.player.playing && this.player.dispatcher) {
                    ...
                    msg.channel.send(createInfoEmbed('Pozostały czas', `${elapsed} / ${media.duration}`));
                } else if(this.player.queue.first) {
                    msg.channel.send(createInfoEmbed('Pozostały czas', `00:00:00 / ${media.duration}`));
                }
            })
                ...
                if(cmd.arguments.length > 0) {
                    cmd.arguments.forEach(arg => {
                        ...
                        if(parts.length == 2) {
                            ...
                            msg.channel.send(createErrorEmbed(`Nie poprawny format pliku`));
                    });
                }
            })
